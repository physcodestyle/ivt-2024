from config.constants import SECONDS_IN_HOUR, SECONDS_IN_MINUTE, SECONDS_IN_24H
from modules.own_time import OwnTime, init_time, init_utc_time_hms


def to_hms(seconds_from_midnight: int) -> tuple[int, int, int]:
  hours = seconds_from_midnight // SECONDS_IN_HOUR
  minutes = (seconds_from_midnight % SECONDS_IN_HOUR) // SECONDS_IN_MINUTE
  seconds = (seconds_from_midnight % SECONDS_IN_HOUR) % SECONDS_IN_MINUTE
...
def to_unix_time_stamp(time: OwnTime) -> int:
  return time[2] + SECONDS_IN_MINUTE * time[1] + SECONDS_IN_HOUR * time[0]


def to_unix_time_stamp_from_utc(time: OwnTime) -> int:
  return to_unix_time_stamp(time) + SECONDS_IN_MINUTE * time[4] + SECONDS_IN_HOUR * time[3]
...
def add_seconds_offset(current_time: OwnTime, interval_in_seconds: int) -> tuple[OwnTime, int]:
  ...
  new_time_in_seconds = raw_time_in_seconds % SECONDS_IN_24H
  day_offset = raw_time_in_seconds // SECONDS_IN_24H
